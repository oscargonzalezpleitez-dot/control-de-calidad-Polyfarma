...
// ─── Mapeo de datos del registro → props del certificado ──────────────────────
function buildCertificateProps(rec: any): CertificadoAguaProps {
  ...
  // name → valor resuelto como string
  ...
  const fmt = (v: string | undefined, suffix?: string) =>
    v ? (suffix ? `${v} ${suffix}` : v) : '—';
  ...
  return {
    ...
    fechaAnalisis: byName['fecha_micro']
      ? fmtDate(byName['fecha_micro'])
      : fmtDate(byName['fecha_produccion']),
    responsable:   fmt(byName['analista']),
    fFab:          fmtDate(byName['fecha_produccion']),
    fechaEmision:  rec.completedAt
    ...
    // Fisicoquímicos
    rTemperatura:   byName['temperatura']  ? `${byName['temperatura']} °C`    : '—',
    rConductividad: byName['conductividad'] ? `${byName['conductividad']} μS/cm` : '—',
    rToc:           byName['toc']          ? `${byName['toc']} ppb`           : '—',
    rPh:            fmt(byName['ph']),

    // Microbiológicos
    rBacterias:  byName['bioburden']  ? `${byName['bioburden']} UFC/mL` : '—',
    rHongos:     fmt(byName['staph_aureus']) || fmt(byName['endotoxinas']) || '—',
    rEcoli:      fmt(byName['e_coli']),
    rPseudomona: fmt(byName['pseudomonas']),

    // Pie
    notaMuestra: byName['observaciones'] || 'El resultado corresponde únicamente a la muestra referida',
    codigo:      'CC-F-063',
  };
}
...
export default function CertificadoPage() {
  ...
  // Solo QC-AGUA-001
  if (rec.format?.code !== 'QC-AGUA-001') return (
    <div className="text-center py-16 text-gray-400">
      <AlertTriangle className="w-10 h-10 mx-auto mb-3 opacity-60" />
      <p className="font-medium">Este certificado solo aplica para el formato QC-AGUA-001</p>
